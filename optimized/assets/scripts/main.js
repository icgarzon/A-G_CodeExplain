var test = (function test(){
   ...
   return {
      ...
      getHeadingText:()=>{
        if(typeof $elems.heading != 'undefined' && $elems.heading != null) return $elems.heading.textContent;
        return null;
      },
   ...
   async function init(params={}) {
      ...
      if ($elems?.heading && headingText)
         $elems.heading.textContent = headingText; // The object exists, so we can set the text (content) inside of the element that function gets in first parameter "headingText"
      ...
      if ($elems.features && featuresList) { // Check if the object 'features' exists in the DOM
         ...
         for (featureItem of featuresList) { // Iteration of the elements received in features in the third parameter of the function
            ...
            if (featureItem?.id) $listItem.setAttribute(selectors.dataFeatureId, featureItem?.id); //Create an attribute into the element with the value that comes from id propertie, it's going to add something like data-feature-id="X" in the code
            ...
            $list.appendChild($listItem); // Add the element to the parent wrap we created previous <ul></ul>
